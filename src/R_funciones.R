...
clasificar.negocios <- function(l.JSON, datos = TRUE){
  ...
  if (datos){
    ...
    t_salida <- clasificacion.areas[,list(types, zona1)]
    t_salida[,n_sitios := 0]
    t_salida[,fila     := 0]
    
  }      
  setorder(t_salida, zona1)
  t_salida <- reshape(t_salida[,list(fila, zona1)],v.names='zona1',idvar='fila',timevar="zona1",direction="wide")
  t_salida2 <-t_salida[,lapply(.SD, as.character), .SDcols=2:(ncol(t_salida))]
  
  if (datos){    
      t_salida2[!is.na(t_salida2),]<-1
      t_salida2[is.na(t_salida2),]<- 0
      t_salida2 <- t_salida2[,1:(ncol(t_salida2)-1)]
  }else{
    t_salida2[,]<- 0
    }
