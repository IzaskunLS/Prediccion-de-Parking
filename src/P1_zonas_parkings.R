...
colnames(parkings)<-c('Id',	
                      'Location_latitude', 'Location_longitude', 
                      'Parking_name', 'Direccion', 'Provider_name',
                      'Creacion', 'Ultima_ Act', 'Ciudad', 'Pais', 
                      'Freemium', 'Status')
...
parkings<-merge(parkings, temp.parkings[,list(Id, p.name=Parking_name)],
                by.x = 'Id', by.y = 'Id',
                all.x = TRUE, all.y = FALSE)
parkings<-parkings[is.na(p.name)]
...
for (i in 1:nrow(parkings)){
      ...
      json.parking<-json.parking[,-c('n_sitios.OTROS')]
      json.rv <- tratar_json(latitud, longitud, dist.parking, 'rv_park')
      json.rv<-json.rv[,-c('n_sitios.OTROS')]
      json.rv[, dist.parking := dist.parking]
      
      json.dist.parking <- rbind(json.parking[,1:34], json.rv[,1:34])
      ...
      json.rv[, dist.parking := dist.parking]
      json.rv<-json.rv[,-c('n_sitios.OTROS')]
      json.dist.parking <- rbind(json.dist.parking,json.parking[,1:34], json.rv[,1:34])
      ...
      json.dist.parking<-json.dist.parking[, lapply(.SD, sum), .SDcols=36:37]
